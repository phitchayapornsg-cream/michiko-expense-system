...
function clearForm() {
  ...
  ["paidTo", "billNo", "detail", "preparedBy", "note", "category"].forEach(key => fields[key].value = "");
...
function prepareNextVoucherForm(dateValue = fields.date.value) {
  ...
  ["paidTo", "billNo", "detail", "preparedBy", "note"].forEach(key => fields[key].value = "");
  itemWrap.innerHTML = "";
  addItem();
  receiptData = "";
  updateReceiptPreview();
...
function init() {
  ...
  document.querySelectorAll(".suggest-field").forEach(wrapper => setupSuggestField(wrapper, updateVoucher));
